...
%Scaling:
r= 3.3*10^(-4); % cm, radius of RBC
tau = 9*mu*Lz/(2*(r^2)*drho*g); % sedimentation time of a single RBC
zlist_scaled=[1/2:Nz]'*dz/Lz; % scaled height
time_scaled=[0:Nt]'*dt/tau; % scaled time
...
plot(zlist_scaled,phi(:,1),'linewidth',2,'color',linecolors(1,:));
hold on
...
%     end
    ...
    if ismember(t,tplot)
        if t==Nt
            plot(zlist_scaled,phi(:,1),'linewidth',2,'color',linecolors(Ntplot+1,:));
            hold off
        else
            plot(zlist_scaled,phi(:,1),'linewidth',2,'color',linecolors(find(tplot==t)+1,:));
            hold on
        end
    end
end
...
legend({'t=0',
     sprintf("t= %7.1e",tplot(1)*dt/tau),
    sprintf("t= %7.1e",tplot(2)*dt/tau),
    sprintf("t= %7.1e",tplot(3)*dt/tau),
    sprintf("t= %7.1e",tplot(4)*dt/tau),
    sprintf("t= %7.1e",tplot(5)*dt/tau),
    sprintf("t= %7.1e",tplot(6)*dt/tau),
    sprintf("t= %7.1e",tplot(7)*dt/tau),
    sprintf("t= %7.1e",tplot(8)*dt/tau),
    sprintf("t= %7.1e",tplot(9)*dt/tau),
    sprintf("t= %7.1e",tplot(10)*dt/tau),
    sprintf("t= %7.1e",tplot(11)*dt/tau)
...
xlabel('x','FontSize',30);
ylabel('z','FontSize',30);
title(ax,['t=' num2str(Tf/tau) ''],'FontSize',20)
q=colorbar;
...
figure(4);
ax=gca;
plot(time_scaled, height/Lz, '.', 'markersize', 8);
hold on
plot(time_scaled,hm*ones(1,Nt+1)/Lz, '.', 'markersize', 8);
hold off
set(gca,'FontSize',20)
xlim([0 time_scaled(end)])
ylim([0 1])
legend({'h(t)',
     'h_m'
    },'FontSize',20,'NumColumns',2);
...
% Write data to excel to import in mathematica:
filename = 'Data.xlsx'; %height
writematrix(height/Lz,filename,'Sheet',1)

filename = 'time_Data.xlsx'; %time
writematrix(time_scaled,filename,'Sheet',1)

...
zlist=[1/2:Nz]'*dz; % list of point in z
...
control=drho*dt*g/(dz*eta); % measure of stability
...
% Plot h and phi initial condition:
figure(2);
linecolors = jet(11);
...
height=[zlist(Nz)]; % 'height' of interface plot against time
k=ones(Nz,Nx)*(abs((phim-phi_0)^3)); 
...
for t=1:Nt,
    ...
    for iterz = 1:Nz-1,
        for iterx=1:Nx,
            ...
            dphi=(dt/(eta*dz))*(k(iterz,iterx))*(phi(iterz+1,iterx))*p_z;
    ...
    for iterz = 1:Nz,
        for iterx=1:Nx,
            ...
            elseif phi(iterz,iterx)<=0 % If phi in one of the boxes becomes negative quit run
                som0=som0+1;
                'error0' 
                return
            else % If phi in one of the boxes becomes bigger than the maximum quit run
                som1=som1+1;
                'error1'
                return
            end
    ...
    if ismember(t,tplot)
        if t==Nt
            ...
            plot(zlist,phi(:,1),'linewidth',2,'color',linecolors(t*10/Nt+1,:));
            hold on
        end
    end
end
% Plot layout plots of phi and h:
set(gca,'FontSize',20)
xlim([0 Lz]);
ylim([0 1]);
